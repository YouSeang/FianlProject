	...
	public void execute(Map<String, Object> model) {
		...
		if (existingAudioFiles != null) {
			for (String voicePath : existingAudioFiles) {
				adminDao.insertAudioFile(
						Map.of("id", fileId++, "scenarioName", scenarioName, "voicePath", voicePath, "isFinal", 0));
			}
		}
		...
		if (audioFiles != null) {
			for (MultipartFile file : audioFiles) {
				if (!file.isEmpty()) {
					...
					adminDao.insertAudioFile(
							Map.of("id", fileId++, "scenarioName", scenarioName, "voicePath", fileName, "isFinal", 0));
				}
			}
			...
			// If no new audio files are added, ensure the last existing file is set to
			// isFinal = 1
			int maxId = adminDao.getMaxAudioIdByScenarioName(scenarioName);
			if (maxId > 0) {
				adminDao.updateIsFinal(Map.of("id", maxId, "scenarioName", scenarioName));
