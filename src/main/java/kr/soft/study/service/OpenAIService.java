...
<<<<<<< HEAD
            "나는 너에게 대사 리스트와 하나의 문장을 줄거야. 너가 할 일은 문장과 뜻이 조금이라도 통하거나 동일하거나 포함된다고 생각하는 대사를 리스트에서 하나 고르는 거야. 내가 요청한 것을 수행할 때, 다른 문장들은 말하지 말고 오로지 숫자로만 대답해줘. 예를 들면 6 이런 식으로. 만약에 해당하는 것이 없으면 0을 반환해줘.\n\n" +
            "%s\n\n" +
            "입력: %s\n응답 (숫자로만 대답해줘):", 
            promptFromDB, 
            userInput
