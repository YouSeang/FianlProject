...
<body>
    ...
    <main class="main-content">
        <div class="container mt-5">
            <h2><c:choose><c:when test="${empty video.id}">새 비디오 추가</c:when><c:otherwise>비디오 수정</c:otherwise></c:choose></h2>
            <form action="${pageContext.request.contextPath}/admin/videoSave" method="post">
                <input type="hidden" id="videoId" name="id" value="${video.id}">
                <div class="mb-3">
                    <label for="videoCategory" class="form-label">카테고리</label>
                    <input type="text" class="form-control" id="videoCategory" name="category" value="${video.category}" required>
                </div>
                <div class="mb-3">
