		...
		document.addEventListener('DOMContentLoaded', function () {
			...
			function addMessage(content, type) {
				...
				if (isSpeakingEnabled) {
					speakText(content, type === 'received' ? 'ko-KR' : 'en-US'); // 한국어와 영어 음성 선택
				}
			}
			...
			function speakText(text, voiceLang) {
				var msg = new SpeechSynthesisUtterance();
				var voices = window.speechSynthesis.getVoices();
				msg.voice = voices.filter(function (voice) { return voice.lang === voiceLang; })[0];
				msg.text = text;
			...
			document.getElementById('toggleSwitch').addEventListener('change', function () {
				...
				if (isSpeakingEnabled) {
					speakText(document.getElementById('mobile-content').innerText, 'ko-KR');
				} else {
