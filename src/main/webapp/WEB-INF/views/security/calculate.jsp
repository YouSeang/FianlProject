...
.donation-form {
	background-color: #fffacd; /* 밝은 노란색 */
	padding: 10px; /* 여백 조정 */
	border-radius: 30px;
	width: 100%;
	max-width: 1300px;
...
<body>
	...
	<div class="container mt-5">
		<h2 style="font-size: 40px; text-align: center;">전세 가격 계산기</h2>
		<div class="donation-form p-3 p-xl-4 rounded-6">
			<p style="font-size: 20px;">(근저당권 설정액 + 전체 보증금)이 매매가의 80%를 넘으면
				깡통전세로 보고 있습니다. 이 계산식이 항상 정답은 아니니, 다른 조건도 고려하세요.</p>
		...
		<form id="calculatorForm" method="POST">
			<div class="form-group">
				<label for="propertyType">주택 유형:</label> <select id="propertyType"
					name="propertyType" class="form-control" required>
			...
			<div id="apartmentFields" style="display: none;">
				<div class="form-group">
					<label for="housePrice">주택 시세 (백만원):</label> <input type="number"
						id="housePrice" name="housePrice" class="form-control" required>
				</div>
				<div class="form-group">
					<label for="mortgageAmount">근저당권 설정액 : 소유주 총 대출 금액 (백만원):</label> <input
						type="number" id="mortgageAmount" name="mortgageAmount"
			...
			<div id="villaFields" style="display: none;">
				<div class="form-group">
					<label for="housePrice">주택 시세:실 거래가(백만원):</label> <input
						type="number" id="housePrice" name="housePrice"
						class="form-control" required>
				</div>
				<div class="form-group">
					<label for="mortgageAmount">근저당권 설정액 : 소유주 총 대출 금액 (백만원):</label> <input
				...
				<div class="form-group">
					<label for="totalOtherDepositAmount">타 거주자 총 보증금 (백만원):</label> <input
						type="number" id="totalOtherDepositAmount"
						...
						function() {
							$('#propertyType').change(
									function() {
										...
										$('#apartmentFields').find('input')
												.removeAttr('required').prop(
														'disabled', true);
										$('#villaFields').find('input')
												.removeAttr('required').prop(
														'disabled', true);

										// 선택된 필드만 보이게 하고 required 속성 추가
										if (selectedType == "apartment") {
											$('#apartmentFields').show().find(
													'input').attr('required',
													true).prop('disabled',
													false);
											$('#villaFields').hide();
										} else if (selectedType == "villa") {
											$('#villaFields').show().find(
													'input').attr('required',
													true).prop('disabled',
													false);
											$('#apartmentFields').hide();
										} else {
											$('#apartmentFields, #villaFields')
													.hide();
										}
									});
